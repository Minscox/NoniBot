...
const { description } = require("./openticket");

module.exports = {
  
    run: async(client, message, args) => {

        if (!args[0]) return message.reply("**Veuillez entrer un rôle!**")
        let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[0]) || message.guild.roles.cache.find(r => r.name.toLowerCase() === args.join(' ').toLocaleLowerCase());
        if (!role) return message.reply("**Veuillez entrer un rôle valide!**");
    ...
    name: 'roleinfo',
    aliases: [],
    description: 'Permet de connaître les informations (permissions...) d\' rôle.'
}